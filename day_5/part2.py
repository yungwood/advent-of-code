...
parser.add_argument('--debug', action='store_true', help='Increase logging output verbosity')
try:
...
def process_map(maps, seed):
    ...
    for map in maps:
        ...
        if seed >= map[1] and seed < (map[1] + map[2]):
            ...
            log.debug("{} becomes {} ({} to end of array)".format(seed, new_value, remaining_offset))
            return [new_value, remaining_offset]
    # if there is no array that starts after this seed then return 0 for offset
    if not range_starts:
        range_starts = [0]
    log.debug("{} remains {} ({} to start of next array)".format(seed, seed, min(range_starts)))
...
def seed_to_location(map_data, seed):
    ...
    log.debug("* Seed {} = Location {}, skippable={}".format(seed, location[1], skippable))
    ...
    for seed in seeds:
        ...
        while i <= seed[1]:
            ...
            if not smallest or value[0] < smallest:
                log.debug("{} is now the smallest location value!".format(value[0]))
                smallest = value[0]
